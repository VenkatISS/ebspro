...
//		var val = Math.floor(Math.random() * (ele.length + 1));
		var val = Math.floor(Math.random() * 11);
//		var val = "Prod"+(ele.length+ +1);
		...
		a.innerHTML = "<td><select name='pid' id='pid' onchange='checkForDeletedItems()' class='form-control input_field select_dropdown ic sadd'>"+ctdatahtml+"</select>" +
				"<input type=hidden name='uniqRandomValForProd' id='uniqRandomValForProd' value="+val+" >" +
				"</td>";
	...
	if(ele.length==1){
		...
		if(agency_stock_data) {
			for(var s=0;s<agency_stock_data.length;s++){
				if((prodId != "") && (agency_stock_data[s].prod_id == prodId) && (agency_stock_data[s].prod_code == prod)){
...
//						var date = (new Date(effdate)).toISOString().slice(0,10);
//						var date = (new Date(effdate)).toLocaleString().slice(0,10);
		...
		for(var e=0;e<ele.length;e++){
			...
			var invNo=""; var editYrN = 1; var prodId = "";
