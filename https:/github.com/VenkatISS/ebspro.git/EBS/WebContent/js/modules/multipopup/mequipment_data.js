...
mpopcdatahtml = "<OPTION VALUE='0'>SELECT</OPTION>";
if(cat_cyl_types_data.length>0) {
	for(var z=0; z<cat_cyl_types_data.length; z++){
		mpopcdatahtml=mpopcdatahtml+"<OPTION VALUE='"+cat_cyl_types_data[z].id+"'>"+cat_cyl_types_data[z].cat_name+"-"+cat_cyl_types_data[z].cat_desc+"</OPTION>";
	}
}

var tbody = document.getElementById('mequipopup_page_data_table_body');
for(var f=equipment_data.length-1; f>=0; f--){
	if(equipment_data[f].deleted==0) {
		var tblRow = tbody.insertRow(-1);
		tblRow.style.height="20px";
   		tblRow.align="left";
   		var spd = fetchProductDetails(cat_cyl_types_data, equipment_data[f].prod_code);
	   	var ed = new Date(equipment_data[f].effective_date);
   		tblRow.innerHTML = 	"<td>" + spd +  "</td>" + 
   							"<td>" + eus[equipment_data[f].units] + "</td>" + 
   							"<td>" + equipment_data[f].gstp +  "</td>" + 
   							"<td>" + equipment_data[f].csteh_no +  "</td>" + 
   							"<td>" + equipment_data[f].security_deposit +  "</td>" + 
   							"<td>" + equipment_data[f].os_fulls +  "</td>" + 
   							"<td>" + equipment_data[f].os_emptys +  "</td>" + 
   							"<td>" + ed.getDate()+"-"+months[ed.getMonth()]+"-"+ed.getFullYear() + "</td>" 
   		/*+ "<td align='center'><img src='images/delete.png' onclick='mequipopupdeleteItem("+equipment_data[f].id+")'></td>"*/;
	}
};

function addmEquipopupRow() {
	document.getElementById('mequipopup_page_add_table_div').style.display="block";
	document.getElementById('mequipopupsavediv').style.display="inline";
    $("body").addClass('sidebar-collapse').trigger('collapsed.pushMenu');

    var trcount = document.getElementById('mequipopup_page_add_table_body').getElementsByTagName('tr').length;
    if(trcount>0) {
    	var trv=document.getElementById('mequipopup_page_add_table_body').getElementsByTagName('tr')[trcount-1];
	...
	var ele = document.getElementsByClassName("eqic");
	if(ele.length < 4){
		var tbody = document.getElementById('mequipopup_page_add_table_body');
		...
		a.innerHTML = "<td valign='top' height='4' align='center'><select name='eqepid' id='eqepid' onchange='checkFormEquipopupDeletedProducts(this)' class='form-control input_field select_dropdown eqic sadd'>"+mpopcdatahtml+"</select></td>";
...
function checkFormEquipopupDeletedProducts(eqpobj) {
	var formobj = document.getElementById('mequipopup_page_add_form');
	...
	if(ele.length==1){
		...
		for(var d=0;d<del_equipment_data.length;d++) {
			if(del_equipment_data[d].prod_code == parseInt(prod)) {
				...
				formobj.eqepid.selectedIndex=prodval;
		...
		for(var e=0;e<ele.length;e++){
			...
			for(var d=0;d<del_equipment_data.length;d++){
				if(del_equipment_data[d].prod_code == parseInt(prod)) { 
					...
					enableSelect(sprod,sprod.length);		
					formobj.eqepid[e].selectedIndex=prdval;
					disableSelect(sprod,sprod.length);
...
function mequipoupsaveData(obj){
	var formobj = document.getElementById('mequipopup_page_add_form');
	var ems = "";	
	if(document.getElementById("eqepid") != null){
		
		var elements = document.getElementsByClassName("eqic");
		if(elements.length==1) {
			var eitem = formobj.eqepid.selectedIndex;
			...
			ems = mEquipopupvalidateEntries(eitem,eusitem,egst,ecsteh,esd,ef,ee,ed);
		}else if (elements.length>1){
			for(var i=0; i<elements.length; i++){
				var eitem = formobj.eqepid[i].selectedIndex;
...
/*function mequipopupdeleteItem(id) {
...
function mEquipopupvalidateProduct2(text) {
	if (equipment_data.length != 0) {
		for (var i = 0; i < equipment_data.length; i++) {
			var spd = fetchProductDetails(cat_cyl_types_data,equipment_data[i].prod_code);
...
function mEquippopupvalidateProduct() {

	var e2 = document.getElementsByClassName("form-control input_field select_dropdown eqic");
	if (e2.length == 1) {
		var e = document.getElementById("eqepid");
		var text = e.options[e.selectedIndex].text;
		var flag = mEquipopupvalidateProduct2(text);
		return flag;
	} else if (e2.length > 1) {
		var flag = false;
		var h = 1;
		for (i = 0; i < e2.length - 1; i++) {
			...
			var flag = mEquipopupvalidateProduct2(e3);
			if (flag) {
				for (j = 0; j < e2.length - 1; j++) {
					var e4 = e2[j + 1].options[e2[j + 1].selectedIndex].text;
					flag = mEquipopupvalidateProduct2(e4);
					var k = 0;
					if (flag) {
...
function mEquipopupvalidateEntries(item,units,gstp,cstehno,sd,fs,es,ed){
	var formobj = document.getElementById('mequipopup_page_add_form');
	var errmsg = "";

	if (!mEquippopupvalidateProduct())
