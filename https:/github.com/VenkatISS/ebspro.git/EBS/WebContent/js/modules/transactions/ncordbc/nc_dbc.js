		...
		if(bom_data[a].id == bidsv){
			...
			for(var b=0; b<bomItems.length; b++) {
				...
				tblRow.innerHTML = "<tr><td><input type='text' class='form-control input_field' id='pd' name='pd' title='" + spd + "' value='"+spd+"'> <input type='hidden' class='form-control input_field' id='pid' name='pid' title='" + bomItems[b].prod_code + "' value='"+bomItems[b].prod_code+"'> </td>" 
					...
					+ "<td> " +
							"<input type='text' class='form-control input_field' id='pa' name='pa' size='8' value='' readonly='readonly' style='background-color:#F3F3F3;' placeholder ='PRODUCT AMOUNT'> " +
...
*/
	...
	if(!(aro.length>0) && (ctype != 3 && ctype != 4 && ctype != 5))
		errmsg = errmsg+"Please enter AMOUNT RECEIVED ONLINE<br>";
	else if(validateDot(aro) && (ctype != 3 && ctype != 4 && ctype != 5)) 
	...
	else if(cashr == 0 && (ctype == 4 || ctype == 5))
	     errmsg = errmsg+"Please Enter Cash RECEIVED.It Cannot be 0<br>";
	else if((ctype == 3 || ctype == 4 || ctype == 5) && aro !=0){
		 errmsg = errmsg+"AMOUNT RECEIVED ONLINE must be 0 For UJJWALA and BPL<br>";
	}
	
	
	var ttlamt= Math.round(parseFloat(cashr)+parseFloat(aro)+parseFloat(ula));
	if((errmsg == "") && (parseFloat(ttlamt) != parseFloat(invamt)) && ((cashr !=0 || aro != 0 ) && ula == 0))
...
function calculateValues() {
	...
	if(elements.length==1) {
		...
		if(erms.length>0) {
			...
			formobj.bp.value = round(((ofpbpv-updv)*qty*nocs),2);
			formobj.optaxablea.value = round(((ofpbpv-updv)*qty*nocs),2);
			...
			if(pid==10 ||pid==11 || pid==12){
				...
				formobj.opcgsta.value = "NA";
				formobj.opsgsta.value = "NA";
				formobj.opigsta.value = "NA";
				formobj.optota.value = ((nocs * (ofpbpv-updv) * qty)).toFixed(2);
				...
				formobj.opcgsta.value = (nocs * (opgsta/2)).toFixed(2);
				formobj.opsgsta.value = (nocs * (opgsta/2)).toFixed(2);
				formobj.opigsta.value = "0.00";
				formobj.optota.value = (+(nocs * (ofpbpv-updv) * qty) + +(formobj.opsgsta.value) + +(formobj.opcgsta.value)).toFixed(2);				
			}
			formobj.tdup.value = updv.toFixed(2);
		}
		...
		for(var i=0; i<elements.length; i++){
			...
			if(erms.length>0) {
				...
				formobj.optaxablea[i].value = round(((ofpbpv-updv)*qty*nocs),2);
				...
				if(pid==10 ||pid==11 || pid==12){
					...
					formobj.opcgsta[i].value = "NA";
					formobj.opsgsta[i].value = "NA";
					formobj.opigsta[i].value = "NA";
					formobj.optota[i].value = ((nocs * (ofpbpv-updv) * qty)).toFixed(2);
					...
					var opgsta = ((((ofpbpv-updv)*qty)*vpv)/100).toFixed(2);
					formobj.opcgsta[i].value = (nocs * (opgsta/2)).toFixed(2);
					formobj.opsgsta[i].value = (nocs * (opgsta/2)).toFixed(2);
					formobj.opigsta[i].value = "0.00";
					formobj.optota[i].value = (+(nocs * (ofpbpv-updv) * qty) + +(formobj.opsgsta[i].value) + +(formobj.opcgsta[i].value)).toFixed(2);
				}

				totdis = +totdis + +(updv);
			}
		}
		formobj.tdup.value = totdis.toFixed(2);
	}
	
	
	formobj.tdamt.value = (nocs * totalDAmt).toFixed(2);
...
function fetchOfpRefillUnitPriceForNCDBC(pc_data, pcid) {
	for(var i=0; i< pc_data.length; i++){
		if(pc_data[i].prod_code == pcid) {
...
function fetchOfpARBUnitPrice(pc_data, pcid) {
	for(var i=0; i< pc_data.length; i++){
...
function setOfpUnitPriceValue(pidv) {
	var upv;
	if(pidv < 10) {
		upv = fetchOfpRefillUnitPriceForNCDBC(refill_prices_data,pidv);
		return upv;
	}else if(pidv <= 12) {
		return 0;		
	}else if(pidv <= 100) {
		...
		upv = fetchOfpARBUnitPrice(arb_prices_data,pidv);
		return upv;
	}
}
...
function fetchPriceAndVAT() {
	...
	for(var i=0; i<elements.length; i++){
		var pidv = formobj.pid[i].value;
//		var gstv = pidv <= 12 ? fetchGSTPercentForNCDBC(equipment_data,pidv) : ( pidv <= 100 ? 18 : fetchARBGSTPercent(arb_data,pidv));
		var gstv = pidv <= 12 ? fetchGSTPercentForNCDBC(equipment_data,pidv) : ( pidv <= 100 ? fetchServicesGSTPercent(pidv) : fetchARBGSTPercent(arb_data,pidv));
		var upv = setUnitPriceValue(pidv);
		if((gstv !="false")) {
			if((upv != "false")) {
				...
				if(millisecs < prodDateInMaster){
					...
					formobj.gstp[i].value = gstv;
					formobj.up[i].value = upv;
					formobj.opbasicprice[i].value = setOfpUnitPriceValue(pidv);
...
function fetchServicesGSTPercent(pidv) {
	var gstp = 18;
	if(pidv == 30 || pidv == 31)
		gstp = 0;
