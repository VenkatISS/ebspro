var tbody = document.getElementById('mareacode_page_data_table_body');
for(var f=area_codes_data.length-1; f>=0; f--){
   var tblRow = tbody.insertRow(-1);
   tblRow.style.height="20px";
   tblRow.align="left";
 //  var ihtml = "<td></td>";
   var ed = new Date(area_codes_data[f].effective_date);
   tblRow.innerHTML = 
	   "<td>"+ area_codes_data[f].area_code +"</td>"+
	   "<td>"+ area_codes_data[f].area_name +"</td>"+
	   "<td>"+ area_codes_data[f].one_way_dist +"</td>"+
	   "<td>"+ area_codes_data[f].transport_charges +"</td>"+	
	   "<td>"+ ed.getDate()+"-"+months[ed.getMonth()]+"-"+ed.getFullYear() +"</td>"/*+
	   "<td align='center'><img src='images/delete.png' onclick='mAreaCodedeleteItem("+area_codes_data[f].id+")'></td>"//+ihtml
*/};


function addmAreaCodeRow(){
	var x = document.getElementById('mareacode_page_add_table_div');
    $("body").addClass('sidebar-collapse').trigger('collapsed.pushMenu');

    if (x.style.display === 'none')
        x.style.display = 'block';
	document.getElementById('mareacode_popupsavediv').style.display="inline";

    var trcount = document.getElementById('mareacode_page_add_table_body').getElementsByTagName('tr').length;
    if(trcount>0){
    	var trv=document.getElementById('mareacode_page_add_table_body').getElementsByTagName('tr')[trcount-1];
	...
	if(ele.length < 4){
		var tbody = document.getElementById('mareacode_page_add_table_body');
...
function savemAreaCodeData(obj) {
	var formobj = document.getElementById('mareacode_page_data_form');
	var ems = "";
	...
	if (document.getElementById("ac") != null) {
		...
		if (elements.length == 1) {
			...
			ems = mAreaCodevalidateEntries(ac, an, owt, tc, ed);
...
/*function mAreaCodedeleteItem(id) {
...
function mAreaCodevalidateEntries(ac, an, owt, tc, ed) {
	var formobj = document.getElementById('mareacode_page_data_form');
	var errmsg = "";
