...
function addRow() {
	document.getElementById('refillOfpNote').style.display="block";
...
function saveData(obj) {
	...
	if (document.getElementById("pid") != null) {
		...
		if (elements.length == 1) {
			...
			ems = validateRefillEntries(epid, ersp, esgst, ecgst,gstpercent, ebp, emon,eyr,eyrv,eofp);
...
function calculateValues() {
	...
	if (document.getElementById("rsp") != null) {
		...
		if (elements.length == 1) {
			...
			var ems = validateRefillEntries2(epid,rspv,ofpv);
			if (ems.length > 0) {
				//alert(ems);
				document.getElementById("dialog-1").innerHTML = ems;
				alertdialogue();
			...
			var ofpGstp = calculateGSTPrice(ofpv, pid);
...
function validateRefillEntries(prod, rsp, sgst, cgst,gstpercent, bp, mon, yr, yrv, ofp) {
	var errmsg = "";
	var formobj = document.getElementById('page_data_form');
...
function validateRefillEntries2(pid,rsp,ofp) {
	var errmsg = "";
	var formobj = document.getElementById('page_data_form');
