...
if(checkNum=="1"){
	...
	for(var f=page_data.length-1; f>=0; f--) {
		...
		tblRow.innerHTML = "<tr>"+
			...
			"<td align='center'>"+ page_data[f].offer_price +"</td>"+
...
function addRow() {
	document.getElementById('arbOfpNote').style.display="block";
...
function saveData(obj) {
	...
	if(document.getElementById("pid") != null) {
		...
		if (elements.length == 1) {
			...
			ems = validateArbEntries(epid, ersp, emon, eyr,eyrv, esgst, ecgst,gstpercent,ebp, eofp);
...
function validateArbEntries(prod, rsp, mon, yr,yrv, sgst, cgst,gstpercent, bp, ofp) {
	var errmsg = "";
	var formobj = document.getElementById('page_data_form');
...
function validateArbEntries2(rsp,ofp) {
	var errmsg = "";
	var formobj = document.getElementById('page_data_form');
	...
	if (document.getElementById("rsp") != null) {
		...
		if (elements.length == 1) {
			...
			if ((formobj.pid.selectedIndex > 0) && (rspv.length > 0)) {
				...
				var ems = validateArbEntries2(rspv,ofpv);				
				if (ems.length > 0) {
					//alert(ems);
					document.getElementById("dialog-1").innerHTML = ems;
					alertdialogue();
				...
				var opGstp = calculateGSTPrice(ofpv, pid);
