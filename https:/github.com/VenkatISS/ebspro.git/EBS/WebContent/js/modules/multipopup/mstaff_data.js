var tbody = document.getElementById('staff_mStaffpopup_data_table_body');
for(var f=staff_data.length-1; f>=0; f--){
	
	if(staff_data[f].deleted == 0) {
...
//    var ihtml = "<td></td>";
	...
	tblRow.innerHTML ='<td>' + staff_data[f].emp_code +'</td>'+
   	...
   	//	+'<td valign="top" height="4" align="center"><img src="images/delete.png" class="fa fa-trash-o" onclick="deletemStaffPopupItem('+staff_data[f].id+')"></td>'//+ihtml	   
	}
};
 
function addmStaffPopupRow() {
	document.getElementById('myStaffDataDIV').style.display = 'block';
	document.getElementById('mstaff_popupsavediv').style.display="inline";
    $("body").addClass('sidebar-collapse').trigger('collapsed.pushMenu');

    var trcount = document.getElementById('mpopup_staff_add_table_body').getElementsByTagName('tr').length;
    if(trcount>0){
    	var trv=document.getElementById('mpopup_staff_add_table_body').getElementsByTagName('tr')[trcount-1];
	...
	if(ele.length < 4){
		var tbody = document.getElementById('mpopup_staff_add_table_body');
...
function savemStaffpopupData(obj) {
	var formobj = document.getElementById('mpopupstaff_data_form');
	var ems = "";
	...
	if (document.getElementById("s_code") != null) {
		...
		if (elements.length == 1) {
			...
			ems = validatemStaffPopupSDEntries(escode, esname, esdob, esdesig, esrole);
...
/*function deletemStaffPopupItem(staffDataId) {
...
function deletemStaffPopupItem(id){
	 $("#myDialogText").text("Are You Sure You Want To Delete?");
	 var formobj = document.getElementById('mpopupstaff_data_form');
	 confirmDialogue(formobj,3543,id);
}

function validatemStaffPopupSDEntries(code, name, dob, designation, role) {
	var errmsg = "";
