...
/*		if((GSTIN != "NA") || (custName == "Cash Sales"))
...
for(var n=0; n<page_data.detailsList.length; n++) {
	...
	var tvpp = fetchGSTPercent(equipment_data, page_data.detailsList[n].prod_code); 
	var ur = page_data.detailsList[n].unit_rate;
	var dur = page_data.detailsList[n].disc_unit_rate;
	var ppc = page_data.detailsList[n].pre_cyls;
	...
	if(qty == 0) {
		...
		if((parseFloat(page_data.detailsList[n].cgst_amount)==0) && (parseInt(tvpp)!=0) && (parseFloat(page_data.detailsList[n].igst_amount)!=0)) {
			tgsta = page_data.detailsList[n].igst_amount;
		}else if((parseFloat(page_data.detailsList[n].cgst_amount)!=0) && (parseInt(tvpp)!=0) && (parseFloat(page_data.detailsList[n].igst_amount)==0)) {		
			tgsta = +(page_data.detailsList[n].cgst_amount)+ +(page_data.detailsList[n].sgst_amount);
		}else if(parseInt(tvpp)==0)
			tgsta = 0;
	}
