...
function addProduct(et){
    ...
    var isbpl = document.getElementById("ctypebpl").checked;
    ...
    if(isnc){
    	...
    	isbplV = document.getElementById("ctypebpl").value;
    	isncV="";
    	isdbcV="";
    	isujwlV="";
    	isujwcV="";
		...
		if(et=='s'){
			...
			if(sp.selectedIndex>0){
				...
				if((isnc==false || isnc==undefined) && (isdbc==false || isdbc==undefined) && 
						(isujwl==false || isujwl==undefined) && (isujwc==false || isujwc==undefined) && (isbpl==false || isbpl==undefined)) {
					document.getElementById("dialog-1").innerHTML = "Please select Connection type.";
					alertdialogue();
					return false;
                }else if((isnc==true || isdbc==true) && (spidv==26 || spidv==27 || spidv==30 || spidv==31)){
					document.getElementById("dialog-1").innerHTML = "BPL AND Ujjwala CONNECTION Charges are not allowed for NC and DBC type Connections .";
					alertdialogue();
					return false;
					...
					alertdialogue();
					return false;
                }else if((isujwl==true || isujwc==true) && (!(spidv==30 || spidv==31)) ){
                	document.getElementById("dialog-1").innerHTML = "Please select only Ujjwala Charges for Ujjwala type of connection.";
					alertdialogue();
					return false;
                }
                else if((isbpl==true) && (!(spidv==26 || spidv==27)) ){
                	document.getElementById("dialog-1").innerHTML = "Please select only BPL Charges for BPL type of connection.";
					alertdialogue();
					return false;
                }
...
function saveData(obj){
	...
	var isnc = document.getElementById("ctypen").checked;
    var isdbc = document.getElementById("ctyped").checked;
    var isctypeul = document.getElementById("ctypeul").checked;
    var isctypeuc = document.getElementById("ctypeuc").checked;
    var isbpl = document.getElementById("ctypebpl").checked;
    ...
    if(isnc){
    	...
    	isucV = document.getElementById("ctypeul").value;
    	isncV="";
    	isdbcV="";
    	isucV="";
    	isbplV="";
    	...
    	isucV = document.getElementById("ctypeuc").value;
    	isncV="";
    	isdbcV="";
    	isulV="";
    	isbplV="";
    ...
    document.getElementById("ctypeul")
...
//	var redp = document.getElementsByClassName("crd")[1].selectedIndex;
	...
	var OnlyBplSrv = pval.filter(function( s ) { 
		return s.indexOf('BPL')>=0; 
	});
	var OnlyUjwalaSrv = pval.filter(function( s ) { 
		return s.indexOf('UJJWALA')>=0; 
	});
	
	if((isnc== true || isdbc==true) && ((OnlyBplSrv.length >= 1) || OnlyUjwalaSrv.length >=1))
		str = str + "BPL  and Ujjwala CONNECTION Charges are not allowed for NC and DBC type Connections.";

/*---------------------------------------------------------------------------------UJJWALA  Conditions-----------------------------------------------------------------------------------------*/
	if((document.getElementById("ctypeul").checked == true) || (document.getElementById("ctypeuc").checked == true) || (document.getElementById("ctypebpl").checked == true)){  //ujjwala if start
	
		var OnlyUjwalaDom = pval.filter(function( s ) { 
			return s.indexOf('14.2 KG NS')>=0; 
		});
		var OnlyUjwalaDom5 = pval.filter(function( s ) { 
			return s.indexOf('5 KG SUB')>=0; 
		});
		...
		if((OnlyUjwalaDom.length <1 && OnlyUjwalaDom5.length < 1) || (OnlyUjwalaSCReg.length <1)){
			if(OnlyUjwalaDom.length <1 && OnlyUjwalaDom5.length < 1){
				str = str + "You can only have DOMESTIC-5 Kg SUB and DOMESTIC-14.2 Kg NS CYLINDER For BPL and Ujjwala. <br>";
			}
			
			if(!(OnlyOneReg.length >=1))
				str = str +  "Please add Regulator-SC TYPE For Ujjwala and BPL.<br>";
			else if(!(OnlyUjwalaSCReg.length >=1))
				str = str +  "You can only have Regulator-SC TYPE For BPL and Ujjwala. <br>";
		}else if(ctype == 3 || ctype == 4 || ctype == 5) {
			var eqdp = document.getElementsByClassName("crd")[0].selectedIndex;
			var redp = document.getElementsByClassName("crd")[1].selectedIndex;
			if(eqdp != 2 || redp != 2)
				str = str + "You can only have DEPOSIT REQUIRED No For Equipment and Regulator in Ujjwala and Bpl.  <br>";
		}
		
		if(!(OnlyUjwalaSrv.length >=1) && (ctype==3 || ctype==5))
			str = str +  "You can either have Installation Charges For Ujjwala or DGCC Book Charges For Ujjwala<br>";
		
		if(!(OnlyBplSrv.length >=1) && (ctype==4))
			str = str +  "You can either have Admin Charges For BPL or DGCC Book Admin Charges For BPL<br>";
		...
		if(!(OnlyUjwalaARBSur.length >=1))
			str = str + "You should have SURAKSHA For Ujjwala and Bpl. <br>";
		else if((OnlyUjwalaARBLIGH.length >= 1) || (OnlyUjwalaARBKIT.length >= 1) || (OnlyUjwalaARBOTH.length >= 1))
			str = str + "You can have only STOVE along with SURAKSHA For Ujjwala and Bpl. <br>";
	}
	//ujwala if end 
