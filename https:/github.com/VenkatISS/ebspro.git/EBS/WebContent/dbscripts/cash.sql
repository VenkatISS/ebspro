...
INSERT INTO CASH_TRANS_ENUM (ID, TRANS_MODULE) VALUES (12,'SALE RETURN');
...
/*SALE RETURN INSERT*/
delimiter |
CREATE  TRIGGER SALES_RETURN_INVOICES_TRIG
    AFTER INSERT ON SALES_RETURN_DATA
    FOR EACH ROW 
BEGIN
   if  NEW.PAYMENT_TERMS = 1 
   then
   SET @Y = (SELECT CASH_BALANCE FROM AGENCY_CASH_DATA WHERE CREATED_BY = NEW.CREATED_BY and T_DATE <= NEW.INV_DATE and TRANS_TYPE != 11 order by T_DATE desc, ID desc limit 1) - NEW.INV_AMT;
   INSERT INTO AGENCY_CASH_DATA(INV_NO,TRANS_TYPE,T_DATE,CASH_AMOUNT,CASH_BALANCE,CREATED_BY,CREATED_DATE,MODIFIED_BY,MODIFIED_DATE,VERSION,DELETED,DISCOUNT) VALUES(NEW.SR_NO, 12, NEW.INV_DATE, NEW.INV_AMT,@Y,NEW.CREATED_BY,NEW.CREATED_DATE,0,0,1,0,0);
   end if;
END;
|
delimiter ;
