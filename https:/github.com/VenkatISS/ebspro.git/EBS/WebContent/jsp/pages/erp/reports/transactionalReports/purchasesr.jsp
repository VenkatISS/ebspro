...
-->
		...
		<title>MyLPGBooks DEALER APPLICATION - PURCHASE REPORTS<%  out.println("-- FROM DATE:"+ request.getAttribute("fdate") +" & TO DATE :"+ request.getAttribute("tdate")); %>&nbsp &nbsp&nbsp :: PRODUCT / VENDOR SELECTED : &nbsp:&nbsp</title>
		...
		<jsp:useBean id="cpreport_data" scope="request" class="java.lang.String"></jsp:useBean>
		<jsp:useBean id="arbpreport_data" scope="request" class="java.lang.String"></jsp:useBean>	
		...
		<script type="text/javascript">
			...
			var report_data = <%= cpreport_data %>;
			var arb_report_data = <%= arbpreport_data %>;
			var rrtype = <%= rrt %>;
			var ctdatahtml = "";
			var vendatahtml = "";
			var pcodep="";
		...
		<script type="text/javascript">
			...
			vendatahtml = "<OPTION VALUE='-1'>SELECT</OPTION>";
			if(cvo_data.length>0) {
				for(var z=0; z<cvo_data.length; z++){
//					if((cvo_data[z].deleted == 0)&&(cvo_data[z].cvo_cat != 1)){
// 					if((cvo_data[z].deleted == 0) && (cvo_data[z].cvo_cat != 1) && (cvo_data[z].cvo_cat != 3)){
					if((cvo_data[z].cvo_cat != 1) && (cvo_data[z].cvo_cat != 3)){
						if(cvo_data[z].cvo_cat != 2){
							...
							vendatahtml=vendatahtml+"<OPTION VALUE='"+cvo_data[z].id+"'>"+omc_name+"-"+cvo_data[z].cvo_name+"</OPTION>";
						}
					}
				}
			}
...
<%-- 	<script src="js/commons/checkBrowser.js?<%=randomUUIDString%>"></script> --%>
		        	...
		        	<div class="clearfix"></div>
		          	<div class="col-md-15">
		  				<form id="purchase_search_form" name="" method="post" action="ReportsDataControlServlet">
							...
							<input type="hidden" id="page" name="page" value="jsp/pages/erp/reports/transactionalReports/purchasesr.jsp">
							<input type="hidden" id="actionId" name="actionId" value="7002">
 							...
 							<div class="col-md-2 reprt">
 								<label>VENDOR NAME </label>
  								<select class="form-control input_field select_dropdown" name="pv" id="pv">
									<%  String vndr="<script>document.writeln(vendatahtml)</script>";
										out.println("value: "+vndr); %>
  								</select>
 							</div>
 						</form>	
  						<button onclick="fetchPurchaseReport()" id="searchb" name="searchb" class="btn btn-info color_btn" style="margin-top:20px;">SEARCH</button>
		  			</div>
				</div>
				<br/>
				<div class="row">
	     		 	 ...
	     		 	 <div class="col-md-12"> 
	      				...
	      				<div>
								<p><span id="fromd" ><b> FROM DATE:</b></span>&nbsp&nbsp&nbsp&nbsp&nbsp<%  out.println( request.getAttribute("fdate")); %>
        	      				...
        	      				&nbsp&nbsp&nbsp&nbsp <b>PRODUCT / VENDOR SELECTED :</b> <span id="type_selected"></span>
						</div>
						<table class="table table-striped" id="m_data_table">
        	      					<thead>
            	    						...
            	    						<tr class="title_head">
												...
												<th width="10%" class="text-center sml_input">HSN <br>CODE</th>
												<th width="10%" class="text-center sml_input">GST <br>RATE</th>
												<th width="10%" class="text-center sml_input">VENDOR</th>
												<th width="10%" class="text-center sml_input">VENDOR<br>GSTIN</th>
												...
												<th width="10%" class="text-center sml_input">TAXABLE <br> VALUE</th>
    ...
    <script>
	    if((report_data.length >0) || (arb_report_data.length > 0)){           
    		pcodep = <%= request.getAttribute("pcodep")%>;
   	    	vcodep = <%= request.getAttribute("vcodep")%>;
	    }
    </script>
	<script type="text/javascript" src="js/modules/reports/transactionalReports/purchasesr.js?<%=randomUUIDString%>"></script>
...
$(document).ready(function() {
	$('#m_data_table').DataTable( {
		...
		footerCallback: function ( row, data, start, end, display ) {
		    ...
		    $( api.column( 10 ).footer() ).html('total : '+ttaxamt);
		    $( api.column( 11 ).footer() ).html(tigstamt);
		    $( api.column( 12 ).footer() ).html(tcgstamt);
		    $( api.column( 13 ).footer() ).html(tsgstamt);
		    $( api.column( 14 ).footer() ).html(tOtherCharges);
