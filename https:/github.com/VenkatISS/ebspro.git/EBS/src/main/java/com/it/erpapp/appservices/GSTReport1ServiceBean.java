...
public class GSTReport1ServiceBean {

	Logger logger = LoggerFactory.getLogger(GSTReport1ServiceBean.class);
	long agencyId;
	String[] sMonth;
	String[] sYear;
	...
	public void fetchGSTReport1Data(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			avo = (AgencyVO) request.getSession().getAttribute(ApplicationConstants.paramKeys.AGENCYVO.getValue());
			
			ofp_acceptable = ((AgencyVO) request.getSession()
					.getAttribute(ApplicationConstants.paramKeys.AGENCYVO.getValue())).getOffer_price_acceptable();
			...
			List<ServicesGSTData> chargesgst = ppmdf.getAgencyServicesGTData(agencyId);
			...
			List<GSTR1DataDO> b2clList = fetchGSTR1B2CLData(dcsis2, ccsis2, arbsis2, edos, arbdos, prsis2 , request);
			List<GSTR1DataDO> b2csList = fetchGSTR1B2CSData(dcsis3, ccsis3, arbsis3, edos, arbdos, prsis3, serdos,
					ncdbcs, rcs, tvs, chargesgst, request);
	...
	public List<GSTR1DataDO> fetchGSTR1B2CLData(List<DOMSalesInvoiceDO> dcsis, List<COMSalesInvoiceDO> ccsis,
			List<ARBSalesInvoiceDO> arbsis, List<EquipmentDataDO> edos, List<ARBDataDO> arbdos,
			List<PurchaseReturnDO> prsis , HttpServletRequest request) {
				...
				for (EquipmentDataDO edo : edos) {
					...
					for (COMSalesInvoiceDetailsDO dataDO : detailsList) {
						if (edo.getProd_code() == dataDO.getProd_code()) {
							...
							gstDataDO.setInv_no(ccsi.getSr_no());
							gstDataDO.setInv_date(ccsi.getSi_date());
							gstDataDO.setInv_amount(dataDO.getOp_total_amt());
							gstDataDO.setQuantity(dataDO.getQuantity() - dataDO.getPre_cyls());
							gstDataDO.setUnit_rate(dataDO.getOp_basic_price());
							gstDataDO.setTaxable_value(dataDO.getOp_taxable_amt());
							gstDataDO.setInv_type(1);
							gstDataDOList.add(gstDataDO);
						}
					}
	...
	public List<GSTR1DataDO> fetchGSTR1B2CSData(List<DOMSalesInvoiceDO> dcsis, List<COMSalesInvoiceDO> ccsis,
			...
			List<TVDataDO> tvs,List<ServicesGSTData> schargesgst, HttpServletRequest request) {
...
//		For OfferPrice: Currently dom is not considered. Will be used later.
		dcsis.forEach(dcsi -> {
			for (EquipmentDataDO edo : edos) {
				List<DOMSalesInvoiceDetailsDO> detailsList = dcsi.getDetailsList();
				for (DOMSalesInvoiceDetailsDO dataDO : detailsList) {
				...
				for (ServicesDataDO serdo : serdos) {
					...
					for (NCDBCInvoiceDetailsDO detailsDO : ncdbcList) {
						if (serdo.getProd_code() == detailsDO.getProd_code()) {
							GSTR1DataDO gstDataDO = new GSTR1DataDO();
							if(serdo.getProd_code()<=29)
								gstDataDO.setGstp("18");
							else if(serdo.getProd_code()>29)
								gstDataDO.setGstp("0");
							gstDataDO.setInv_no(ncdbc.getSr_no());
							gstDataDO.setInv_date(ncdbc.getInv_date());
							gstDataDO.setQuantity(detailsDO.getQuantity());
				...
				for (EquipmentDataDO edo : edos) {
					...
					for (NCDBCInvoiceDetailsDO detailsDO : ncdbcList) {
						if ((edo.getProd_code() == detailsDO.getProd_code()) && (edo.getProd_code() < 10)) {
							...
							if(edo.getProd_code() <= 3){
								gstDataDO.setUnit_rate(detailsDO.getUnit_rate());
								float tax = Float.parseFloat(detailsDO.getBasic_price());
								String taxable = Float.toString(tax);
								gstDataDO.setTaxable_value(taxable);
							}else {
								gstDataDO.setUnit_rate(detailsDO.getOp_basic_price());
								gstDataDO.setTaxable_value(detailsDO.getOp_taxable_amt());
							}
							gstDataDOList.add(gstDataDO);
						}
					}
...
//							gstDataDO.setTaxable_value(taxable);
			...
			//	for (ServicesDataDO serdo : serdos) {
				for (ServicesGSTData serdo : schargesgst) {
					List<NCDBCInvoiceDetailsDO> ncdbcList = ncdbc.getDetailsList();
					for (NCDBCInvoiceDetailsDO detailsDO : ncdbcList) {
					//	if (serdo.getProd_code() == detailsDO.getProd_code()) {
						if (serdo.getService_product_id() == detailsDO.getProd_code()) {
						    GSTR1DataDO gstDataDO = new GSTR1DataDO();
							gstDataDO.setGstp("18");
							if(serdo.getService_product_id() <= 29)
								gstDataDO.setGstp("18");
							else if(serdo.getService_product_id() > 29)
								gstDataDO.setGstp("0");
							gstDataDO.setInv_no(ncdbc.getSr_no());
							gstDataDO.setInv_date(ncdbc.getInv_date());
							gstDataDO.setQuantity(detailsDO.getQuantity());
				...
				for (EquipmentDataDO edo : edos) {
					...
					if (edo.getProd_code() == rc.getProd_code()) {
						...
						if(edo.getProd_code() <= 3){
							...
							gstDataDO.setInv_amount(rc.getOp_total_amt());
							gstDataDO.setTaxable_value(rc.getOp_taxable_amt());
						}
						gstDataDOList.add(gstDataDO);
					}
				}
			...
			//	for (ServicesDataDO serdo : serdos) {
	            for (ServicesGSTData serdo : schargesgst) {
	            	//	if (serdo.getProd_code() == 23) {
	               if (serdo.getService_product_id() == 23) {
						GSTR1DataDO gstDataDO = new GSTR1DataDO();
						gstDataDO.setGstp("18");
						gstDataDO.setInv_no(String.valueOf(rc.getId()));
						gstDataDO.setInv_date(rc.getRc_date());
		...
		tvs.forEach(tv -> {
			...
			/*	if (serdo.getProd_code() == 20 || serdo.getProd_code() == 28 || serdo.getProd_code() == 29) {
				...
				if (serdo.getService_product_id() == 20 || serdo.getService_product_id() == 28 || serdo.getService_product_id() == 29) {
					if (tv.getTv_charges_type() == serdo.getService_product_id()) {
						gstDataDO.setGstp("18");
	...
	public List<GSTR1HSNDataDO> fetchGSTR1HSNData(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			for (EquipmentDataDO edo : edos) {
				...
				if (pc <= 3) {
//					consolidatedList = srdf.fetchAgencyGSTR1HSNDOMCSalesData(requestParams, agencyId, pc, ofp_acceptable);
					consolidatedList = srdf.fetchAgencyGSTR1HSNDOMCSalesData(requestParams, agencyId, pc);
				}
				if ((pc > 3 && pc <= 9)) {
					consolidatedList = srdf.fetchAgencyGSTR1HSNCOMCSalesData(requestParams, agencyId, pc, ofp_acceptable);
				}
				if ((pc > 9 && pc <= 12)) {
			...
			for (ARBDataDO arbdo : arbdos) {
				...
				consolidatedList = srdf.fetchAgencyGSTR1HSNARBSalesData(requestParams, agencyId, pc, ofp_acceptable);
				Collections.sort(consolidatedList, Comparator.comparing(GSTR1ProductDataVO::getCreatedDate));
				GSTR1HSNDataDO srdvo = new GSTR1HSNDataDO();
...
//					String tv = "0";
					...
					for (GSTR1ProductDataVO psvo : consolidatedList) {
						if (serdo.getProd_code() == psvo.getProd_code()) {
							...
							cgst = Float.toString(Float.parseFloat(cgst) + Float.parseFloat(psvo.getCgst_amount()));
							 */
//							tv = Float.toString(Float.parseFloat(tt) + (Float.parseFloat(psvo.getTotal_value())));
							/*igst = Float.toString(Float.parseFloat(igst) +((Float.parseFloat(serdo.getGst_percent_applicable()))*(Float.parseFloat(psvo.getIgst_amount()))));
							sgst = Float.toString(Float.parseFloat(sgst) +((Float.parseFloat(serdo.getGst_percent_applicable()))*(Float.parseFloat(psvo.getSgst_amount()))));
							cgst = Float.toString(Float.parseFloat(cgst) + ((Float.parseFloat(serdo.getGst_percent_applicable()))*(Float.parseFloat(psvo.getCgst_amount()))));
							
							tt=Float.toString(Float.parseFloat(ttax)+Float.parseFloat(igst)+Float.parseFloat(sgst)+Float.parseFloat(cgst));
							 */
							
		                    for (ServicesGSTData gstcharge : chargesgst) {
		                    	if((serdo.getProd_code()<=28) && (serdo.getProd_code() ==gstcharge.getService_product_id())){
		                    		igst = Float.toString(Float.parseFloat(igst) +((Float.parseFloat(gstcharge.getGst_percent_applicable()))*(Float.parseFloat(psvo.getIgst_amount()))));
		                    		sgst = Float.toString(Float.parseFloat(sgst) +((Float.parseFloat(gstcharge.getGst_percent_applicable()))*(Float.parseFloat(psvo.getSgst_amount()))));
		                    		cgst = Float.toString(Float.parseFloat(cgst) + ((Float.parseFloat(gstcharge.getGst_percent_applicable()))*(Float.parseFloat(psvo.getCgst_amount()))));
		                    		tt=Float.toString(Float.parseFloat(ttax)+Float.parseFloat(igst)+Float.parseFloat(sgst)+Float.parseFloat(cgst));
		                    		break;
		                    	}else if((serdo.getProd_code()>28) && (serdo.getProd_code() ==gstcharge.getService_product_id())){
									tt=Float.toString(Float.parseFloat(ttax)+Float.parseFloat(igst)+Float.parseFloat(sgst)+Float.parseFloat(cgst));
								}
							}
